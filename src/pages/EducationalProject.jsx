...
const tips = [
  {
    name: "Save images/assets in your public folder",
    description:
      "The assets then do not need to be imported into the jsx file for use and can easily be accessed by assigning '/assets/imagename.png' to the img src",
    icon: PhotographIcon,
  },
  {
    name: "Deploying a React App on gh-Pages",
    description:
      'Change your build script in package.json to "build": "react-scripts build && cp build/index.html build/404.html". This allows you to route to different pages without getting a 404 error.',
    icon: CursorClickIcon,
  },
  {
    name: "Routing for Apps hosted using Netlify",
    description:
      "Add a _redirects file (without any extension) to your public folder. Copy paste the following into the file - /* /index.html 200",
  ...
  return (
    <>
      <div className="m-3 p-3">
        ...
        <div className="py-12 bg-teal-700 my-8 rounded">
          <div className="max-w-xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
            <h2 className="mb-3 font-mono font-bold text-2xl text-white">
              React: A few Tips & Tricks
            </h2>
            <p className="space-y-10 lg:space-y-0 lg:grid lg:grid-cols-1 lg:gap-8">
              {tips.map((feature) => (
                <div key={feature.name}>
                  <dt>
                    <div className="flex items-center justify-center h-12 w-12 rounded-md bg-green-200 text-blueGray-700">
                      <feature.icon className="h-6 w-6" aria-hidden="true" />
                    </div>
                    <p className="mt-5 text-lg leading-6 font-medium text-purple-50">
                      {feature.name}
